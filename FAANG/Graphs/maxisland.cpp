...
map<int , vector<int> > buildGraph(int** mat , int n , int m) {
...
void dfs ( int i ,map<int , vector<int> >  adjList , int &currcomp){
...
int maxIsland(int** mat , int n , int m ) {
	...
	for(auto node : adjList ){
		if(visited[node.first] == false  ){
			int currcomp = 0;
			dfs( node.first , adjList , currcomp ) ;
			count = max(count , currcomp);
...
int main(){
    ...
    cout<<(maxIsland(matrix, n , m ))<<endl;
    return 0;
}
